...
unittest {
    // Test applyMask (symmetric)
    ubyte[] data = [0x48, 0x65, 0x6C, 0x6C, 0x6F];  // "Hello"
    ubyte[4] key = [0x37, 0xFA, 0x21, 0x3D];

    ubyte[] original = data.dup;
    applyMask(data, key);
...
unittest {
    // Test encodeFrame - simple unmasked frame (server→client)
    Frame frame;
    frame.fin = true;
    ...
    assert(encoded[0] == 0x81);  // FIN=1, opcode=1 (text)
    assert(encoded[1] == 0x05);  // MASK=0, len=5
    assert(encoded[2 .. $] == cast(ubyte[]) "Hello");
}
...
unittest {
    ...
    frame.masked = false;
    frame.payload = new ubyte[](256);

    auto encoded = encodeFrame(frame);

    assert(encoded.length == 4 + 256);  // 2 byte header + 2 byte extended len + payload
    assert(encoded[0] == 0x82);  // FIN=1, opcode=2 (binary)
    assert(encoded[1] == 126);   // Extended 16-bit length marker
    assert(encoded[2] == 0x01);  // 256 >> 8
    assert(encoded[3] == 0x00);  // 256 & 0xFF
}

unittest {
    // Test decodeFrame - simple masked frame (client→server)
    // Frame: FIN=1, opcode=1 (text), MASK=1, len=5, mask key, masked "Hello"
    ubyte[4] maskKey = [0x37, 0xFA, 0x21, 0x3D];
    ubyte[] maskedPayload = cast(ubyte[]) "Hello".dup;
    ...
    assert(result.bytesConsumed == data.length);
}

unittest {
    // Test decodeFrame - incomplete frame
    ubyte[] data = [0x81];  // Only 1 byte, need at least 2
...
unittest {
    // Test decodeFrame - unmasked frame rejected in server mode
    ubyte[] data = [
        0x81,  // FIN=1, opcode=1
        0x05,  // MASK=0, len=5
        'H', 'e', 'l', 'l', 'o'
    ...
    assert(threw, "Should throw on unmasked frame in server mode");
}

unittest {
    // Test decodeFrame - unmasked frame accepted in client mode
    ubyte[] data = [
        0x81,  // FIN=1, opcode=1
        0x05,  // MASK=0, len=5
        'H', 'e', 'l', 'l', 'o'
    ...
    auto result = decodeFrame(data, false);  // requireMasked=false
    assert(result.success);
    assert(result.frame.payload == cast(ubyte[]) "Hello");
}

unittest {
    // Test control frame validation - payload too large
    Frame frame;
    frame.fin = true;
    ...
    assert(threw, "Should throw on oversized control frame");
}

unittest {
    // Test control frame validation - fragmented
    Frame frame;
    frame.fin = false;  // Not final = fragmented
    frame.opcode = Opcode.Close;
    frame.payload = [0x03, 0xE8];  // Normal close code
    ...
    assert(threw, "Should throw on fragmented control frame");
}
...
unittest {
    ...
    original.payload = cast(ubyte[]) "Test data 123".dup;
...
unittest {
    // Test large payload encoding (64-bit length)
    Frame frame;
    frame.fin = true;
    frame.opcode = Opcode.Binary;
    frame.masked = false;
    frame.payload = new ubyte[](70000);  // > 65535, requires 64-bit length

    auto encoded = encodeFrame(frame);

    // Header: 2 bytes base + 8 bytes extended length = 10 bytes
    assert(encoded.length == 10 + 70000);
